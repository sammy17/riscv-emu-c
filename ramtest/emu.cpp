...
int main(int argc, char** argv){
    ...
    write_double(MTIME_ADDR, memory, (uint64_t)0);
    write_double(MTIMECMP_ADDR, memory, (uint64_t)(-1));
    uint32_t imm=0;
    ...
    while (1){
        ...
        write_double(MTIME_ADDR, memory, mtime);
        ...
        instruction = read_word(PC_phy, memory);//getINST(PC_phy/4,&memory);
        ...
        switch(opcode){
                ...
                if ((load_addr != FIFO_ADDR_RX) && ((load_addr != FIFO_ADDR_TX))){
                    {
                        ...
                        load_data = read_double(load_addr_phy, memory); //memory.at(load_addr_phy/8);
                    ...
                    if (store_addr_phy >= DRAM_BASE){ // mapping to RAM
                        store_addr_phy = store_addr_phy - DRAM_BASE; // mapping to emulator array memory
                          if (store_addr_phy >= ((1llu)<<MEM_SIZE)){
                                ...
                                store_data = read_double(store_addr_phy, memory); //memory.at(store_addr_phy/8);
                                ...
                                if (!ls_success){
                                    ...
                                    write_double(store_addr_phy, memory, wb_data); //memory.at(store_addr_phy/8) = wb_data;
                                }
                            }
                    }
                    else{ // mapping to peripheral
            ...
            case amo :
                ...
                if (func3 == 0b010) { //AMO.W-32
                    ...
                    switch (amo_op){
                        ...
                        case 0b00011 : //SC.W
                            if (amo_reserve_valid && (reg_file[rs1]==amo_reserve_addr)){
                                ...
                                if (!ls_success){
                                    ...
                                    write_double(load_addr_phy, memory, wb_data); //memory.at(load_addr_phy/8) = wb_data;
                                    ret_data = 0;
                                }
                            }
                            else {
                        ...
                        case 0b00001 : //AMOSWAP.W
                            ...
                            if (!ls_success){
                                ...
                                write_double(load_addr_phy, memory, store_data);
                            }
                            break;
                        ...
                        case 0b00000 : //AMOADD.W
                            ...
                            if (!ls_success){
                                ...
                                ls_success = store_word(load_addr,load_data,wb_data, store_data);
                                write_double(load_addr_phy, memory, store_data);//memory.at(load_addr_phy/8) = store_data;
                            }
                            break;
                    ...
                    wb_data = read_double(load_addr_phy, memory); //memory.at(load_addr_phy/8);
                    switch (amo_op){ 
                        case 0b00010 : //LR.D
