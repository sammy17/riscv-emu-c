...
static uint_t clint_read( uint_t offset)
{
    ...
    case 0xbff8:    //rtc time
        val = read_double(MTIME_ADDR, memory);
        break;
    case 0x4000:    //timecmp
        val = read_double(MTIMECMP_ADDR, memory);
        break;
    default:
        val = 0;
        break;
...
static void clint_write(uint_t offset, uint_t val)
{
    switch(offset) {
        case 0x4000:
            write_double(MTIMECMP_ADDR, memory, val) ;
            mip.STIP=0;
            break;
        default:
            break;
...
extern "C" uint32_t mem_read32(uint64_t addr){
    uint64_t load_data = read_double((addr - DRAM_BASE), memory); //memory.at((addr - DRAM_BASE)/8);
    uint64_t wb_data = 0;
...
extern "C" void mem_write32(uint64_t addr, uint64_t data){
    ...
    store_data = read_double((addr - DRAM_BASE), memory); //memory.at((addr - DRAM_BASE)/8);
    uint64_t wb_data = 0;
    store_word(addr, store_data, data, wb_data);
    write_double((addr - DRAM_BASE), memory, wb_data);//memory.at((addr - DRAM_BASE)/8) = (  wb_data);
}

extern "C" uint16_t mem_read16(uint64_t addr){
    uint64_t load_data = read_double((addr - DRAM_BASE), memory);
    uint64_t wb_data = 0;
...
extern "C" void mem_write16(uint64_t addr, uint64_t data){
    uint64_t store_data = read_double((addr - DRAM_BASE), memory); //uint64_t store_data = memory.at((addr - DRAM_BASE)/8);
    uint64_t wb_data = 0;
...
extern "C" uint8_t mem_read8(uint64_t addr){
    uint64_t load_data = read_double((addr - DRAM_BASE), memory);//uint64_t load_data = memory.at((addr - DRAM_BASE)/8);
    uint64_t wb_data = 0;
...
uint_t translate(uint_t virtual_addr, ttype_t translation_type, plevel_t current_privilage){
    ...
    switch(satp.MODE){
        ...
        case 8 : //Sv39
            ...
            pte2.write_reg( read_double(pte_addr, memory) );
            ...
            if ( (pte2.R==1) | (pte2.X==1) ){ //pte is a leaf
                ...
                pte1.write_reg( read_double(pte_addr, memory) ); //pte1.write_reg(memory.at(pte_addr/8));
                ...
                if ( (pte1.R==1) | (pte1.X==1) ){ //pte is a leaf
                    ...
                    pte0.write_reg(read_double(pte_addr, memory));
